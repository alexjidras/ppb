...
scrolldiv.addEventListener("scroll", ()=> {
	...
	mainchart.setJSONData({
	    ...
	    "categories": [
        {
            "category": data.slice(start,end).map((el, i)=>({ label: i%12 === 0 ? [el["Luna"].capitalize().slice(0,3), el["An"]].join("<br><br><br>") : el["Luna"].capitalize().slice(0,3)}))
        }],
        "dataset": [
        {
            "seriesname": "Pret mediu benzina",
	    ...
	    "seriesname" : "Pret Plafon",
	    data: data.slice(start,end).map((el, i)=> el["Pret_Produs"] !== null
	    	? ({ label: i%12 === 0 ? [el["Luna"].capitalize().slice(0,3), el["An"]].join("<br><br><br>") : el["Luna"].capitalize().slice(0,3), value: el["Pret_Plafon"]}) 
	    	: ({ label: i%12 === 0 ? [el["Luna"].capitalize().slice(0,3), el["An"]].join("<br><br><br>") : el["Luna"].capitalize().slice(0,3),value: el["Pret_Prognoza"], dashed: 1}) )
	    }]
	});
});
